...
module Actionable
  ...
  class Runner
    ...
    # @return [void]
    def run_steps
      @instance.class.steps.each { |step| call_step(step) }
    end
    ...
    # @return [void]
    def run_lifecycle
      outcome_hooks = @result.success? ? @instance.class.success_hooks : @instance.class.failure_hooks
      Actionable.catch_halt do
        outcome_hooks.each { |hook| run_hook(hook) }
        @instance.class.always_hooks.each { |hook| run_hook(hook) }
      end
    end
    ...
    # @return [void]
    def call_step(step)
      step.call(@instance) unless step.skip?(@instance)
    end
